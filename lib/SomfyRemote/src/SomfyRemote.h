...
#ifndef SOMFY_REMOTE_H
#define SOMFY_REMOTE_H
...
public:
  ...
  uint16_t sendButton(int      remote,
                      byte     button,
...
private:
  ...
  uint16_t buildFrame(int      remote,
                      byte     button,
                      uint16_t rollingCode,
                      byte    *frame);
  ...
  void sendLevel(byte level,
                 int  duration);
  void sendLevelChange(byte initState,
                       int  duration1,
                       int  duration2);
  void sendCommand(byte *frame,
                   byte  sync);
};

#endif // ifndef SOMFY_REMOTE_H
